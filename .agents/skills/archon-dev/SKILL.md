...
description: |
  ...
  IMPLEMENT   — "implement the plan", "execute .Codex/archon/plans/auth.plan.md",
...
- **Artifacts**: !`ls .Codex/archon/ 2>/dev/null || echo "none yet"`
- **Active plans**: !`ls .Codex/archon/plans/*.plan.md 2>/dev/null | head -5 || echo "none"`
...
All artifacts go to `.Codex/archon/`. Create subdirectories as needed on first use.
...
- **Conventions**: Read AGENTS.md for project-specific rules
...
4. **Respect AGENTS.md**: Project conventions override cookbook defaults
5. **No AI attribution**: Never add "Generated with Codex" or "Co-Authored-By: Codex" to commits or PRs
