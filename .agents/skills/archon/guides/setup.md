...
```
Header: "Archon skill"
Question: "Would you like to copy the Archon skill into your target repo so Claude Code or Codex can invoke Archon workflows from there?"
Options:
  1. "Yes, copy the skill" — Copies the mirrored Archon host skill into both .claude/skills/archon/ and .agents/skills/archon/. This will appear in git unless you gitignore it.
...
```bash
mkdir -p <target-repo>/.claude/skills <target-repo>/.agents/skills
cp -r <archon-repo>/.agents/skills/archon <target-repo>/.agents/skills/archon
cp -r <archon-repo>/.agents/skills/archon <target-repo>/.claude/skills/archon
```
...
- `.claude/skills/archon/` and `.agents/skills/archon/` — mirrored host skill roots for Claude and Codex
