...
| `model` / `provider`                         | yes     | yes     | ignored | ignored | yes                          | ignored        | ignored |
...
| Codex `modelReasoningEffort` / `webSearchMode` / `additionalDirectories` | yes | yes | ignored | ignored | workflow/config only | ignored | ignored |
...
| Tune Codex reasoning/search/extra dirs for one AI node | Node-level `modelReasoningEffort`, `webSearchMode`, `additionalDirectories` on `command`/`prompt` |
...
1. **`hooks` / `mcp` / `skills` / `output_format` / `allowed_tools` / `denied_tools` on a loop, bash, script, approval, or cancel node** → silently ignored.
2. **Codex `modelReasoningEffort` / `webSearchMode` / `additionalDirectories` on a loop node** → accepted by schema only when represented as workflow-level config; loop execution uses workflow/config defaults, not per-loop-node tuning.
...
| `effort` / `thinking`           | yes           | use `modelReasoningEffort` on workflow or command/prompt nodes | via `effort:` (maps to thinking level) |
