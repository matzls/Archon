    ...
    - hooks:
        - type: prompt
          prompt: |
            ...
            2. A startup command (CLAUDE_CODE_TASK_LIST_ID=<id> Codex) was provided
...
3. **Log the session mapping** — write the mapping to `.Codex/archon/sessions/`:

   ```bash
   mkdir -p .Codex/archon/sessions
   echo '{"session": "${CLAUDE_SESSION_ID}", "task_list": "<TASK_LIST_ID>", "saved_at": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' \
     >> .Codex/archon/sessions/task-lists.jsonl
   ```

4. **Verify the SessionStart hook is installed** — the project-level
   `.Codex/settings.json` already includes a SessionStart hook that runs
       ...
       "SessionStart": [
         {
           "hooks": [
             {
               "type": "command",
               "command": ".Codex/skills/save-task-list/hooks/verify-task-list.sh",
...
5. **Output the startup command** for the user:
   ...
   CLAUDE_CODE_TASK_LIST_ID=<task_list_id> Codex
   ```
