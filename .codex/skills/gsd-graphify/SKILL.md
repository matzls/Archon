...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-sdk query`. Use `node .codex/get-shit-done/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-sdk query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node .codex/get-shit-done/bin/gsd-tools.cjs config-set graphify.enabled true
...
```bash
node .codex/get-shit-done/bin/gsd-tools.cjs graphify query <term>
```
...
```bash
node .codex/get-shit-done/bin/gsd-tools.cjs graphify status
```
...
```bash
node .codex/get-shit-done/bin/gsd-tools.cjs graphify diff
```
...
```
PREFLIGHT=$(node ".codex/get-shit-done/bin/gsd-tools.cjs" graphify build)
```
...
Project root: ${CWD}
gsd-tools path: .codex/get-shit-done/bin/gsd-tools.cjs
...
node \".codex/get-shit-done/bin/gsd-tools.cjs\" graphify build snapshot
