...
- What we're doing: turn `S3` from the PRD execution map into a focused executable slice without widening scope.
...
- What "done" looks like: the repo ships `S3` exactly within `Execution Map row` plus the Execution Map note: typed phase-gate support where runtime permits and proves it with focused validation.
...
- Treat this slice as a scoped receipt for one PRD row: the plan should explain exactly how `S3` lands without borrowing work from later rows.

## 1) Problem Statement
`Archon PIV Loop Codex V2` is already split into execution slices, but `S3` needs a focused plan that keeps the implementation boundary aligned with the PRD instead of expanding into adjacent slice work. The PRD already names the intended surface for this slice; this seeded draft turns that row into an executable workflow artifact.

## 2) Solution Concept (High-Level)
- Keep `S3` anchored to `Execution Map row` plus the Execution Map note: typed phase-gate support where runtime permits.
- Ground the slice against the current repo code, docs, and tests before changing implementation.
- Prefer the smallest producer/consumer boundary that satisfies `S3` without opening neighboring slices early.
- Use focused repo-local validation and doc sync as the initial proof shape for this slice.

## 3) Scope / Non-Goals
- In scope: typed phase-gate support where runtime permits
- In scope: code, contract docs, and tests directly required by `Execution Map row`.
- Non-goal: widening `S3` into neighboring execution-map rows during the first refine/freeze pass.
...
```mermaid
flowchart LR
    A["PRD S3 scope"] --> B["Focused slice plan"]
...
|--------|------|------|--------|
| A1. Keep `S3` scoped to `Execution Map row` and the row note | Preserves deterministic sequencing and protects neighboring slices | Optional polish can wait for later slices | Low |
| A2. Widen `S3` into adjacent concerns during the first pass | May reduce a later handoff | Breaks slice isolation and makes orchestration less predictable | High |
...
| P1 | Proposed | - P1-T1: Implement the smallest load-bearing `S3` surface — proposed<br>- P1-T2: Add or update focused validation for the touched surface — proposed |
...
- [ ] P0-T1: Ground the current slice boundary against repo reality
  - Test Impact: N/A
  - Commands to Run:
    - inspect `docs/prd/r001-archon-piv-loop-codex-v2.md` and the directly implicated repo surfaces for `S3`
    - record the current runtime, contract, and doc truth that this slice must preserve or change
  - Exit Criteria:
    - the plan captures the current repo truth for `S3`
...
- [ ] P0-T2: Lock the slice contract and doc surface
  ...
  - Commands to Run:
    ...
    - identify the exact docs and contracts that must stay in sync with `S3`
  - Exit Criteria:
    - the plan, code boundary, and doc surface describe the same slice
    - no first-pass scope gap remains inside `Execution Map row`
  - Verify Commands:
    - `rg -n "S3|Execution Map row" docs`

### Phase 1 — Scoped Implementation

- [ ] P1-T1: Implement the smallest load-bearing `S3` surface
  - Test Impact: update
  - Commands to Run:
    - modify only the code and docs required to land `S3`
    - keep neighboring execution-map rows untouched unless the PRD contract proves coupling
  - Exit Criteria:
    - typed phase-gate support where runtime permits
...
- [ ] P1-T2: Add or update focused validation for the touched surface
  ...
  - Exit Criteria:
    - the load-bearing behavior of `S3` has direct validation coverage
...
- [ ] P2-T1: Reconcile docs and prove final slice evidence
  ...
  - Exit Criteria:
    - docs, code, and tests agree on the landed `S3` surface
...
- Specs / contracts (if any):
  - use the active design/spec docs directly implicated by `S3` during P0 grounding
- Goals:
...
- Testing (only if code changes):
  ...
  - Test suite status: use the nearest existing repo-local validation surface for `S3`
  - Primary test command(s): record the focused command selected during P0 grounding and keep it tied to the touched surface
  - Test locations: the nearest existing tests for the code touched by `S3`
...
- Definition of Done: `S3` lands exactly within the PRD row boundary and is proven with focused validation evidence.
- Metrics:
  - slice scope stays within `Execution Map row`
  - validation covers the load-bearing behavior changed by `S3`
...
- Initial:
  ...
  - `docs/plans/r001-archon-piv-loop-codex-v2-s3_plan.md` — focused slice execution ledger (accessed 2026-04-27)
...
| --- | --- | --- | --- |
| README | `README.md` | review-only | Check whether `S3` changes operator-facing workflow or setup guidance. |
| Env contract (.env.example) | `.env.example` | N/A | Update only if this slice introduces or changes a runtime env contract. |
| Integration docs | `docs/design/` | review-only | Review the active design docs directly implicated by `S3`. |
| API docs/schema docs | `docs/specs/` | review-only | Promote to must-edit only for the exact contract docs touched by this slice. |
| Specs/contracts (docs/specs/) | `docs/specs/` | review-only | Keep spec updates scoped to the load-bearing contract for `S3`. |
| Reference docs (docs/reference/) | `docs/reference/` | N/A | Update only if `S3` changes a stable operator runbook. |
...
| Capabilities payload docs/schema | `docs/specs/output-format.md` | review-only | Review if `S3` touches emitted payload or packet shape. |
