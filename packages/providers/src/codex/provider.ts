 ...
 * Request-level overrides (from workflow YAML, surfaced via top-level options
 * fields) take precedence over assistantConfig defaults.
 */
function buildThreadOptions(
  ...
  return {
    ...
    modelReasoningEffort: requestOptions?.modelReasoningEffort ?? config.modelReasoningEffort,
    webSearchMode: requestOptions?.webSearchMode ?? config.webSearchMode,
    additionalDirectories: requestOptions?.additionalDirectories ?? config.additionalDirectories,
  };
}
...
/**
 * Build the full env map for a Codex subprocess, applying GitHub CLI auth
 * policy when requested. Returns `undefined` when no env mutation is required
 * so callers can preserve the singleton Codex-client fast path.
 */
async function buildCodexProcessEnv(
  ...
  if (githubCliAuthPolicy === 'inherit') {
    return options?.env ? buildCodexEnv(options.env) : undefined;
  }
...
export class CodexProvider implements IAgentProvider {
  ...
  private async createCodexClient(
    ...
    if (!codexEnv || Object.keys(codexEnv).length === 0) {
  ...
  async *sendQuery(
    ...
    const codexEnv = await buildCodexProcessEnv(cwd, requestOptions);
    const codex = await this.createCodexClient(codexConfig.codexBinaryPath, codexEnv);
