...
export interface AgentRequestOptions {
  ...
  /**
   * GitHub CLI auth policy for assistant subprocesses. `prefer-stored` removes
   * `GH_TOKEN` / `GITHUB_TOKEN` from the subprocess env when `gh` has stored
   * auth for the target host, preventing env-token shadowing during workflow-
   * driven `gh` usage. Structurally matches `@archon/git` GitHubCliAuthPreference.
   */
  githubCliAuthPolicy?: 'inherit' | 'prefer-stored' | 'prefer-env';
  /**
   * Codex-only effective tuning resolved by the workflow executor (workflow-
   * level YAML overriding `.archon/config.yaml` defaults). Providers that
   * don't understand these fields ignore them. Kept at top-level so the
   * dag-executor can express precedence without mutating assistantConfig.
   */
  modelReasoningEffort?: 'minimal' | 'low' | 'medium' | 'high' | 'xhigh';
  webSearchMode?: 'disabled' | 'cached' | 'live';
  additionalDirectories?: string[];
