...
async function executeBashNode(
  ...
  const subprocessEnv = buildNodeSubprocessEnv(artifactsDir, logDir, baseBranch, envVars);
...
async function executeLoopNode(
  ...
  for (let i = startIteration; i <= loop.max_iterations; i++) {
    ...
    if (!iterationIdleTimedOut && (cleanOutput || fullOutput).trim() === '') {
...
async function executeApprovalNode(
  ...
  emitApprovalPendingEvent(workflowRun.id, node.id, renderedMessage, approvalSnapshot ?? {});
